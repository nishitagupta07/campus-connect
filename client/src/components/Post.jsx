...
import API from "../api/Axios"; // ✅ Import API with token
...
export default function Post() {
  ...
  useEffect(() => {
    dispatch(fetchPosts(filter)); // token automatically sent via axios interceptor
  }, [dispatch]);

  const onSubmit = async (e) => {
    e.preventDefault();
    try {
      await API.post("/posts", { ...form, year: Number(form.year) || undefined });
  ...
  return (
    <div className="w-full">
      ...
      <div className="max-w-5xl mx-auto mt-6 p-4 rounded-xl bg-slate-900/80 text-white">
        ...
        <Link to="Addpost" className="hover:text-purple-400 transition">Addpost</Link>
        </div> */}
